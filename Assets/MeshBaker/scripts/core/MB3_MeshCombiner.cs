	...
	*/
	...
	public abstract class MB3_MeshCombiner{
		...
		protected virtual bool _CreateTemporaryTextrueBakeResult(GameObject[] gos, List<Material> matsOnTargetRenderer){ 
            ...
            {
                Debug.LogError("Can't add objects if there are already objects in combined mesh when 'Material Bake Result' is not set. Perhaps enable 'Clear Buffers After Bake'");
                return false;
            }
