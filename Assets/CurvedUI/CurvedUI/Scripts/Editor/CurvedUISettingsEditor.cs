    ...
    void DrawControlMethods()
        {
            ...
            switch (CurvedUIInputModule.Controller)
            {
                case CurvedUIInputModule.CurvedUIController.MOUSE:
                {
					...
					GUILayout.Label("Enabling this control method will disable GoogleVR Gaze support.", EditorStyles.helpBox);
                ...
                {
					#if CURVEDUI_GOOGLEVR
					GUILayout.Label("Enabling this control method will disable GoogleVR Gaze Reticle support. It's counter-intuitive, but if you're working with daydream controller - you want this.", EditorStyles.helpBox);
		...
		{
			GUILayout.BeginVertical();
			GUILayout.Label("Press the button below to make CurvedUI recompile for this control method. Afterwards, you'll see its settings here.", EditorStyles.helpBox);
